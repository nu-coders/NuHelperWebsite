    ...
    Object.keys(table).map((key) => {
      ...
      if (nextSlot === undefined) {
        ...
        if (currSlot['status'] === false && nextSlot['status'] === false) {
          if (
            currSlot['course'] === nextSlot['course'] &&
            currSlot['section'] === nextSlot['section']
          ) {
            result[newKey] = currSlot;
            result[newKey]['startSlot'] = slots[key];
            result[newKey]['endSlot'] = slots[nextKey];
        ...
        {Object.keys(table2).map((key) => (
          <Paper
            ...
            sx={{
              p: 2,
              m: 2,
              backgroundColor: colors[table[key].status ? 'Vacant' : 'Occupied'],
              alignItems: 'center',
            }}
            alignItems='center'
            justify='center'
          >
            <Typography variant='h6' align='center'>
              Status: {table[key].status ? 'Vacant' : 'Occupied'}
            </Typography>
            <Typography variant='h6' align='center'>
              Occupied Until: {table[key]['O/V']}
            </Typography>
            {!table[key].status && (
              <>
                <Typography variant='h6' align='center'>
                  Course: {table[key]['course']}
                </Typography>
                <Typography variant='h6' align='center'>
                  ...
                  <Grid container spacing={3}>
                    <Grid item xs={12} md={8} lg={9}>
                      <Autocomplete
                        ...
                        renderInput={(params) => <TextField {...params} label='Search Rooms' />}
                      />
                    </Grid>
